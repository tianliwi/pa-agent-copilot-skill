...
检查仓库和 venv 是否就绪，如果不存在则自动克隆并安装依赖：

```powershell
# Windows
$REPO_ROOT = if ($env:PA_AGENT_REPO) { $env:PA_AGENT_REPO } else { "$env:USERPROFILE\.pa-agent-repo" }

if (-not (Test-Path "$REPO_ROOT\pa_agent")) {
    Write-Host "首次运行，正在克隆仓库..."
    git clone https://github.com/tianliwi/priceactionanalyzer.git $REPO_ROOT
}
...
Linux/macOS 下：
```bash
REPO_ROOT="${PA_AGENT_REPO:-$HOME/.pa-agent-repo}"
...
> **本地开发**：如果你已经有仓库克隆（如 `C:\playground\pa-agent-copilot-skill`），设置环境变量 `PA_AGENT_REPO` 指向它即可跳过克隆。
